...
const playRandom = async () => {
  if (props.musicList.length === 0) {
    ElMessage.warning('歌单为空，无法随机播放')
    return
  }
  const randomIndex = Math.floor(Math.random() * props.musicList.length) // 随机生成索引
  const song = props.musicList[randomIndex]
  // 调用 handleClick 来播放歌曲，并处理无版权情况
  handleClick(song)

  // 获取播放 store 实例
  const playStore = usePlayStore()
  // 将播放模式设置为随机播放
  playStore.currentPlayMode = playStore.PlayMode.SHUFFLE
  ElMessage.success('已切换到随机播放模式')
}
...
const handleClick = (song) => {
  ...
  playsongs(
    ...
    song.OriSongName || song.songname || formatSongSinger(song.name),
    song.SingerName || song.author_name || formatSongName(song.name),
    song.Image || song.sizable_cover,
  )
}
...
<template>
  ...
  <div class="song-list">
    ...
    <div class="header">
      ...
      <div class="controls">
        <!-- 随机播放按钮 -->
        <el-button @click="playRandom" size="small" class="random-btn">
    ...
    <!-- 列表头部 -->
    <el-row class="song-header" :gutter="20">
      <el-col :span="2"></el-col>
      <!-- 空列 -->
      <el-col :span="10" class="header-title">标题</el-col>
      <!-- 标题列 -->
      <el-col :span="8" class="header-title">歌手</el-col>
      <!-- 歌手列 -->
      <el-col :span="4" class="header-title duration-header">时长</el-col>
      <!-- 时长列 -->
    </el-row>
    ...
    >
      <el-col :span="2">
        ...
        <div class="cover-container">
          ...
          <img
            :src="
              song.cover?.replace('{size}', '64') ||
              song.sizable_cover?.replace('{size}', '64') ||
              song.Image?.replace('{size}', '64') ||
              '/default-cover.jpg'
            "
            class="album-cover"
          />
      ...
      <el-col :span="10" class="song-title-col">
        <!-- 歌曲标题 -->
        <span class="song-title">{{
          song.OriSongName || song.songname || formatSongSinger(song.name) || '歌曲无版权.........'
        }}</span>
        <!-- 歌手名称（移动端显示） -->
        <span class="artist-mobile">{{
          song.SingerName || song.author_name || formatSongName(song.name)
        }}</span>
      </el-col>
      <el-col :span="8" class="artist-col">
        <!-- 歌手名称 -->
        {{ song.SingerName || formatSongName(song.name) }}
      </el-col>
      <el-col :span="4" class="duration-col">
        <!-- 歌曲时长 -->
